 ...
 * MakeTrampoline().  We pre-generate trampolines for the two IDs used in
 * scenario-zero (200 = KPM, 300 = RC/Sm) plus a generic fallback.         */
...
E2Termination::StoreAndRegisterCallback (long ranFunctionId, E2TermCallback cb)
{
  ...
  /* Only funcIds with a pre-generated trampoline (200=KPM, 300=RC) are
   * dispatchable; any other id registers the no-op fallback, so its callback
   * would NEVER fire (indications silently stop). Warn loudly — if the live
   * RIC pins KPM to a different funcId, add a TrampolineN + GetTrampoline case.
   * (See Phase-7 handoff: ns-3 registers KPM under 200; the standalone proof
   * used 2 for kpimon-go — reconcile during the live run.) */
  if (ranFunctionId != 200 && ranFunctionId != 300)
    {
...
void E2Termination::DoStart ()
{
  ...
  // registered RAN-function descriptor, and the local client bind port is chosen by the SCTP
  // client, so only the RIC address/port are passed here. Mirrors examples/e2-setup-minimal.cc.
  //
